...
import { 
  ...
  supprimerClient, supprimerFournisseur, import { 
  listeClients, listeFournisseurs, listeProduits, listeUtilisateurs, dashboard,
  ajouterClient, ajouterFournisseur, ajouterItem, ajouterUtilisateur,
  modifierClient, modifierFournisseur, modifierItem, modifierUtilisateur,
  ...
  GET: {
    ...
    'liste_categories': async () => {
      try {
        ...
        console.log('✅ Réponse liste_categories:', categories);
        // Retourner un tableau vide si categories est undefined ou null
        return categories && Array.isArray(categories) ? categories : [];
      } catch (error) {
        console.error('❌ Erreur dans liste_categories:', error);
  ...
  if (isApiRequest) {
    try {
      ...
      if (matchedHandler) {
        ...
        // Vérifier si responseData est undefined ou null
        if (responseData === undefined || responseData === null) {
          console.warn('⚠️ responseData est undefined ou null pour', endpoint);
          responseData = method === 'GET' ? [] : { erreur: 'Aucune donnée retournée', status: 500 };
        }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => {
    ...
    fetch('/api/liste_categories') // Modifier pour tester liste_categories
      .then(response => response.json())
      .then(data => console.log('✅ Test réussi - Données catégories:', data))
